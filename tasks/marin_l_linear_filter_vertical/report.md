...
- CPU: определяется окружением CI
- Количество ядер: определяется окружением
- RAM: определяется окружением
- OS: Linux (CI окружение)

**Инструментарий:**
- Компилятор: определяется CI окружением
- Тип сборки: Release
- MPI: OpenMPI или MS-MPI (в зависимости от платформы)
...
- Оптимизация коммуникационных паттернов
- Динамическая балансировка нагрузки между процессами

## 9. Источники

1. Open MPI Documentation. https://www.open-mpi.org/doc/
2. Message Passing Interface (MPI) Forum. MPI: A Message-Passing Interface Standard, Version 3.1. https://www.mpi-forum.org/docs/mpi-3.1/mpi31-report.pdf
3. Gaussian Blur. Wikipedia. https://en.wikipedia.org/wiki/Gaussian_blur
4. Image Processing Algorithms. Digital Image Processing, Gonzalez & Woods
