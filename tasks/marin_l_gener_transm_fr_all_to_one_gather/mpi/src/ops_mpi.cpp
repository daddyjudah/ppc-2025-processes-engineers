...
void AssembleAtRoot(const std::vector<char> &data, const std::vector<int> &ranks, int size, int block_sz,
  ...
  std::vector<char> full_data(static_cast<size_t>(size) * static_cast<std::size_t>(block_sz), 0);
...
bool MarinLGenerTransmFrAllToOneGatherMPI::RunImpl() {
  ...
  if (rank == input.root) {
    recv_buffer.resize(static_cast<size_t>(input.count) * static_cast<std::size_t>(size) *
